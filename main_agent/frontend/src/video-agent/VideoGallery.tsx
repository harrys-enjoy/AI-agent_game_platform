...
export function VideoGallery({ owner, refreshKey }: { owner?: string; refreshKey?: unknown }) {
  ...
  // New owner, or a task just changed state (e.g. finished) - start over from page 1.
  useEffect(() => {
    setError(null);
    if (!owner) {
      setTasks([]);
    ...
    let cancelled = false;
    listVideoAgentTasks(owner, PAGE_SIZE, 0)
      .then((result) => {
        if (cancelled) return;
  ...
  async function loadMore() {
    if (!owner || !tasks) return;
    setLoadingMore(true);
    try {
      const result = await listVideoAgentTasks(owner, PAGE_SIZE, tasks.length);
