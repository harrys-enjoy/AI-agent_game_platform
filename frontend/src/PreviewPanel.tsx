...
import { parseStoryText } from "./story-import";
...
type PreviewPanelProps = { isGameQa?: boolean };

export default function PreviewPanel({ isGameQa }: PreviewPanelProps) {
  ...
  async function submitStoryChat(event: FormEvent) {
    ...
    const parsed = parseStoryText(storyChat, "chat-story.txt");
    if (!parsed.answer) {
  ...
  async function handleStoryTxt(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    try {
      const parsed = parseStoryText(await file.text(), file.name);
      setStoryChat(parsed.answer);
      if (!parsed.answer) {
  ...
  return <>
    {shouldShowCodePreview(showStoryWorkspace) && (activeChat === "Video Generation" || activeChat === "Development Assistant") && <section className={`preview-panel ${isOpen ? "is-open" : "is-closed"}`}>
